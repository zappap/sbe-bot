...
# Bu fonksiyon dosyaları Google'a yükler. 
# @st.cache_resource sayesinde her soruda tekrar yükleme yapmaz, sadece bir kere çalışır.
@st.cache_resource
def upload_files_to_gemini():
    ...
    # Genellikle PDF'ler için birkaç saniye gerekir.
    while True:
        all_active = True
        ...
        if all_active:
            break
        time.sleep(2) # Hazır değilse 2 saniye bekle
        
    status_area.success("Tüm belgeler analiz edildi ve sisteme eklendi! ✅")
    time.sleep(1)
    status_area.empty() # Mesajı ekrandan kaldır
